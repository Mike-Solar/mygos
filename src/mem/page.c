...
static void* early_alloc(int64_t size) {
	static void* next_addr = memory.base; // 从引导程序获取的可用内存区域
	void* ret = (void *)ALIGN_UP((uint64_t)next_addr, 12);
	next_addr = ret + size;
	return ret;
}
void buddy_init_stage1(void) {
	...
	num=page_count/num;
	// 使用物理地址初始化链表
	uint32_t num_of_page_inited=0;
	for (int i = 0; i <= MAX_ORDER; i++) {
		init_list_head(&memory_zone.free_area[i].free_list);
		memory_zone.free_area->nr_free=num;
		uint32_t num_of_each=kpow(2,i);
		for (int j=0;j<num;j++) {
			list_add(&phys_page_array[num_of_page_inited].list,
				&memory_zone.free_area[i].free_list);
			num_of_page_inited+=num_of_each;
		}
	}
...
void buddy_init_stage2(void) {
	...
	void *delta=(void *)((void *)phys_page_array_virt_addr-(void *)phys_page_array_phys_addr);
#define PHYS_TO_VIRT(phys) ((void*)((uint64_t)(phys) + delta))
	// 将物理地址转换为虚拟地址
	for (int i = 0; i <= MAX_ORDER; i++) {
...
// 物理内存起始地址（由设备树或硬编码确定）
void* phys_mem_start = 0x80000000uL;
void phys_mem_init() {
	// 清零空闲列表
	...
	// 标记已使用的页（如内核代码、保留区域）
	void *kernel_start = 0x80000000uL;    // 内核起始物理地址
	void *kernel_end   = 0x80200000uL;    // 内核结束物理地址
	for (void *pa = kernel_start; pa < kernel_end; pa += PAGE_SIZE) {
